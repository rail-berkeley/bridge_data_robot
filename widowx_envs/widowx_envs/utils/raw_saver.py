...
class RawSaver():
    ...
    def save_traj(self,  # NOQA
        ...
        print("##################################")
        print('saving trajectory number {}'.format(
            count_trajs(self.save_dir) + 1))
        print("##################################")

        igrp = itr // self.ngroup
        group_folder = os.path.join(
            self.save_dir, 'raw/traj_group{}'.format(igrp))
        ...
        if os.path.exists(traj_folder):
            print('trajectory folder {} already exists, deleting the '
                  'folder'.format(traj_folder))
        ...
        if 'images' in obs_dict:
            ...
            for t in range(T):
                for i in range(n_cams):
                    args = AttrDict(
                        i=i, images=images, t=t, traj_folder=traj_folder)
                    save_single(args)
        ...
        if 'depth_images' in obs_dict:
            ...
            for t in range(T):
                for i in range(n_cams):
                    cv2.imwrite('{}/depth_images{}/im_{}.png'.format(
                        traj_folder, i, t), depth_images[t, i])

        if 'language' in obs_dict:
            language = obs_dict['language']
            if T is not None:
                # Make sure the language is labeled per step.
                assert len(language) == T
            path = '{}/language.txt'.format(traj_folder)
...
def save_single(arg):
    cv2.imwrite(
        '{}/images{}/im_{}.jpg'.format(arg.traj_folder, arg.i, arg.t),
        arg.images[arg.t, arg.i, :, :, ::-1])
